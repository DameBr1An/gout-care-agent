...
FOOD_HINTS = {
    "啤酒": "属于当前阶段建议尽量避免的饮品，尤其在发作风险偏高时更应严格控制。",
    ...
    "豆腐": "多数情况下可以适量食用，但仍建议结合当天整体风险控制总量。",
}
...
def build_daily_lifestyle_guidance(context: dict) -> str:
    ...
    return (
        ...
        hydration=risk_data["hydration_advice"],
        diet=risk_data["diet_advice"],
        exercise=risk_data["exercise_advice"],
        goal=risk_data["behavior_goal"],
    )
...
def answer_food_question(question: str, context: dict) -> str:
    for keyword, answer in FOOD_HINTS.items():
        if keyword in question:
            return (
                "{food}：{answer} 当前发作风险为 {risk}，"
                "建议结合今天的症状、饮水和尿酸情况谨慎选择。"
            ).format(
                food=keyword,
                answer=answer,
                risk=context["risk_result"]["attack_risk_level_cn"],
            )
